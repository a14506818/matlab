     ...
     fid_his= fopen(f_his, 'w');
       ...
       for k = 1:Img_num %Img_num  %那些影像要進去 1:6
            ...
            file_experiment = [ file_title '.xls'];
             ...
             T_number =3; 
             [total_secret ,   secret_array, secret_array_String,  sorted_ind_odd, sorted_ind_even ] = ReadSecretImage_withOrignalImage(secretFile, kindImage, 2, fid_his, file_path ) ; % proposed scheme read secret Image   ReadSecretImage_withOrignalImage(secret_file , kindImage, k, fid_his, orginalImage) 
              bpp_S=( total_secret*3/512/512/20 );
              
              [ PSNRs1 PSNRs2] =  Ann_LSBMatching_advance (f_attack, stego_image, f_n, file_path, secret_array, isColorImage, file_experiment,  bpp_S, isRSAttack, total_secret,  sorted_ind_odd, sorted_ind_even);
